...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        [self dismissViewControllerAnimated:YES completion:^{
            ...
            if ([self.rwDelegate respondsToSelector:@selector(dropdownMenu:didSelectMenuItemAtIndex:)]) {
                [self.rwDelegate dropdownMenu:self didSelectMenuItemAtIndex:indexPath.row];
            }
            if (item.action)
            {
                double delayInSeconds = 0.15;
...
+ (instancetype)presentFromViewController:(UIViewController *)viewController withItems:(NSArray *)items align:(RWDropdownMenuCellAlignment)align style:(RWDropdownMenuStyle)style navBarImage:(UIImage *)navBarImage delegate:(id)delegate completion:(void (^)(void))completion {
